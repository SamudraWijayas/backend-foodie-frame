...
const Users = {
    ...
    create: async (name, email, password, role, avatar) => {
        const queryStr = "INSERT INTO users (name, email, password, role, avatar) VALUES (?, ?, ?, ?, ?)";
        try {
            return await query(queryStr, [name, email, password, role, avatar]);
        } catch (error) {
            throw error;
        }
    },
    update: async (id, name, email, password, role, avatar) => {
        // console.log(name)
